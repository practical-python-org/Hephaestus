...
from hephaestus.cogs.utility._DB_Functions import see_top_10
...
class db_commands(commands.Cog, command_attrs=dict(hidden=True)):
    ...
    async def leaderboard(self, ctx):
        ...
        for person_number, person in enumerate(data):
            ...
            embed.add_field(name=f" -- # {person_number} --", value=f"{member.display_name} with {person[1]}", inline=False)
