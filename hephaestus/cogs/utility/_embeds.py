...
def embed_avatar(before, after):
    embed = discord.Embed(title=f"{before} updated their profile picture!", color=discord.Color.dark_grey(), timestamp=datetime.utcnow())
...
def embed_ban(some_member, audit_log_entry):
    ...
    embed.add_field(name=f"Reason:", value=f"{audit_log_entry.reason}", inline=True)
...
def embed_leave(some_member):
    embed = discord.Embed(title="\u200b", description=f"{some_member} has left us.", color=discord.Color.red(), timestamp=datetime.utcnow())
...
def embed_message_delete(some_member, some_author, some_message):
    embed = discord.Embed(
        ...
        description=f"{some_author} deleted a message \nIn {some_message.channel}\nMessage " f"author: {some_message.author}",
        color=discord.Color.dark_red(),
        timestamp=datetime.utcnow(),
    )

    embed.set_thumbnail(url=some_member.avatar)
...
def embed_message_edit(some_username, orig_author, some_message_before, some_message_after):
    ...
    embed.set_thumbnail(url=orig_author.avatar)

    embed.add_field(name="Original message: ", value=some_message_before.content, inline=True)

    embed.add_field(name="After editing: ", value=some_message_after.content, inline=True)
...
def embed_name_change(name_before, name_after, username_before, username_after):
    ...
    embed.set_thumbnail(url=name_after.avatar)
...
def embed_unban(some_member):
    embed = discord.Embed(
        title=f"<:green_circle:1046088647759372388> User Un-Banned", color=discord.Color.red(), timestamp=datetime.utcnow()
    )

    embed.add_field(name=f"{some_member.name} was un-banned.", value="Welcome back.", inline=True)

    return embed


def embed_leaderboard():
    embed = discord.Embed(title=f"{config['name']}'s Top Point earners", color=discord.Color.gold(), timestamp=datetime.utcnow())
...
def embed_user_profile(some_member_info):
    ...
    embed = discord.Embed(title=f"{name}'s Profile", color=discord.Color.green(), timestamp=datetime.utcnow())
    embed.add_field(name=f"Roles: ", value=rolestr, inline=False)
    embed.add_field(name=f"Joined at: ", value=joined_at, inline=False)
    embed.add_field(name=f"Points: ", value=points, inline=False)
